...
crd_path: docs/fabrics.eda.nokia.com/crds/fabrics.eda.nokia.com_fabrics.yaml
...
!!! warning "MTU considerations for OSPF interoperability"
    ...
    Different network operating systems have different default port MTUs. OSPF is notoriously specific when it comes to MTU, and will not establish a session if the signalled MTU is mismatched. 
...
To establish connectivity between multiple datacenters, several options may be considered. Each has their own use-case:
