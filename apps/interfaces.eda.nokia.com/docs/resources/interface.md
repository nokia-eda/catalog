...
crd_path: docs/interfaces.eda.nokia.com/crds/interfaces.eda.nokia.com_interfaces.yaml
...
A Link Aggregation Group (LAG) is an interface with one or more member ports. LAG interfaces can be configured to use the Link Aggregation Control Protocol (LACP).

Members of the same LAG interface can be distributed over multiple network elements. This **does not** enable connectivity between those ports, but rather makes these network elements work together in multi-homing mode, providing node level redundancy for the other end of the link.
