...
crd_path: docs/interfaces.eda.nokia.com/crds/interfaces.eda.nokia.com_breakouts.yaml
...
Typically, these breakout cables are used in top-of-rack switches where 100G ports are overkill for compute connectivity: 32x100G ports can be split out into 128x25G ports, optimizing space by only requiring 32 optical cages instead of 128. The signalling is frequently done over DAC (direct-attach copper) cables, but can also be done using fibers.
