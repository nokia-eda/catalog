...
To facilitate distributed [routing and switching services](-{{docs_base_url}}-/services_eda_nokia_com), network elements need to exchange forwarding and routing information. The Protocols application enables users to create and manage various routing protocols and contains resources that are split between the Overlay Routing and Default Routing categories.
...
On the other hand, the resources listed under the Overlay Routing category[^3] designed to be associated with a custom, non-default VRF.
...
BGP configuration in the Protocols application supports both the Default VRF and Overlay VRF deployments, with comprehensive features for peer management, route reflection, and policy control.

Two routers that exchange routes using the BGP protocol are called **BGP Peers**, and they inherit settings from the **BGP Group** they belong to. To avoid a full-mesh BGP topology (where every router peers with every other router), designated **Router Reflectors** can be configured that all other routers peer with as Route **Reflector Clients**.
...
OSPF configuration in the Protocols application supports both Default VRF and Overlay VRF deployment models, with features for
