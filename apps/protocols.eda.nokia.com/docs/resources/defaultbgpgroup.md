...
crd_path: docs/protocols.eda.nokia.com/crds/protocols.eda.nokia.com_defaultbgpgroups.yaml
...
As the Default BGP Group is not configured on the node before a [`DefaultBGPPeer`](bgppeer.md) references it, there are no prerequisites that must be fulfilled before this resource is created. Although every parameter can be overridden individually by the [`DefaultBGPPeer`](bgppeer.md), a `DefaultBGPGroup` must be created first.
